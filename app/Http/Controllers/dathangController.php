...
class dathangController extends Controller
{
   public function dathang()
	...
	public function themdonhang(Request $request)
    {
		...
		DB::table('tbl_donhang')->insert(['Email' => $request->email,'TenKhachHang'=>$request->hoten,'SDT'=>$request->sdt,'DiaChi'=>$request->diachi,'TongTien'=>Cart::total(0,'',''),'NgayMua'=>Carbon::now()] ); 
		$layid=DB::table('tbl_donhang')->max('ID'); 
		$giohang=Cart::content();
		foreach($giohang as $value)
		{
			DB::table('tbl_chitetdonhang')->insert(['MaBanh'=>$value->id,'SoLuong'=>$value->qty,'GiaTien'=>$value->price,'ID'=>$layid]);
			$soluong=DB::table('tbl_banh')->select('SoLuong')->where('ID',$value->id)->first();
			$soluong=$soluong->SoLuong-$value->qty;
			DB::table('tbl_banh')->where('id', $value->id)-> update(['SoLuong' =>$soluong]);
					
		}
		if(Session::has('user'))
		{
			DB::table('tbl_donhangkh')->insert(['ID'=>Session::get('user'),'MADH'=>$layid]);
		}
		Cart::destroy();
	...
	{	
		...
		if($donhang->Duyet==2)
		{
			DB::table('tbl_hoadon')->insert(['TenKhachHang'=>$donhang->TenKhachHang,'Email'=>$donhang->Email,'DiaChi'=>$donhang->DiaChi,'SDT'=>$donhang->SDT,'TongTien'=>$donhang->TongTien,'NgayLap'=>Carbon::now()]);
			$layid=DB::table('tbl_hoadon')->max('ID'); 
			$chitietdonghang=DB::table('tbl_chitetdonhang')->where('ID','=',$id)->get();
				foreach($chitietdonghang as $value)
				{
					DB::table('tbl_chitiethoadon')->insert(['ID'=>$layid,'MaBanh'=>$value->MaBanh,'SoLuong'=>$value->SoLuong,'GiaTien'=>$value->GiaTien]);
				}
	...
	{	
		...
		if($donhang->Duyet!=3)
		{
			...
			foreach($chitietdonghang as $value)
			{
				$soluong=DB::table('tbl_banh')->where('ID','=',$value->MaBanh)->first();
				$soluong=$soluong->SoLuong+$value->SoLuong;
				DB::table('tbl_banh')->where('id', $value->MaBanh)-> update(['SoLuong' =>$soluong]);
			}
	...
	public function xemchitietdonhang($id)
	{
		...
		return view('dathang.xemchitietdonhang',['donhang' => $donhang,'chitiet'=>$chitietdonghang]);
	}
	public function indonhang($id)
	{
		...
		$chitietdonghang=DB::table('tbl_chitetdonhang')->join('tbl_banh','tbl_chitetdonhang.MaBanh','=','tbl_banh.ID')->where('tbl_chitetdonhang.ID', '=',$id)->select( DB::raw('tbl_chitetdonhang.SoLuong as SoLuong, TenBanh,HinhAnh,tbl_chitetdonhang.GiaTien as GiaTien') )->get();
		return view('hoadon.inhoadon',['hoadon' => $hoadon,'chitiet'=>$chitietdonghang]);
	}
}
